 ...
 */
export const save = (self: FfmpegCommand) => (output: any) => {
  self.output(output).run();
  return self;
};
...
export const screenshots =
  (self: FfmpegCommand) => (config: any, folder: any) => {
    const source = self._currentInput.source;
      ...
      [
        ...
        function replaceFilenameTokens(pattern: any, next: any) {
          if (pattern.match(/%[bf]/)) {
            if (typeof source !== "string") {
              return next(
                new Error("Cannot replace %f or %b when using an input stream")
              );
            }
 ...
 * @return FfmpegCommand
 */
export const concat = (self: FfmpegCommand) => (target: any, options: any) => {
  // Find out which streams are present in the first non-stream input
  const fileInput = self._inputs.filter(function (input: any) {
    return !input.isStream;
  })[0];

  self.ffprobe(self._inputs.indexOf(fileInput), function (err: any, data: any) {
    if (err) {
      return self.emit("error", err);
    }
